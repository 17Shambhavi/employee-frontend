...
function AdminDashboard({ token, onLogout }) {
    ...
    const loadEmployees = async () => {
        const res = await fetch('http://localhost:8080/api/admin/employees', { headers: { 'Authorization': 'Bearer ' + token } });
        const data = await res.json(); setEmployees(data);
    };
    const loadPendingLeaves = async () => {
        const res = await fetch('http://localhost:8080/api/admin/leaves/pending', { headers: { 'Authorization': 'Bearer ' + token } });
        if (res.ok) { const data = await res.json(); setPendingLeaves(data); setStatLeaves(data.length); }
    };
    const loadTodayAttendance = async () => {
        const res = await fetch('http://localhost:8080/api/admin/attendance/today', { headers: { 'Authorization': 'Bearer ' + token } });
        if (res.ok) { const data = await res.json(); setStatPresent(data.presentCount || 0); }
    };
    const loadInsights = async () => {
        const res = await fetch('http://localhost:8080/api/admin/attendance/insights', { headers: { 'Authorization': 'Bearer ' + token } });
        if (res.ok) { const data = await res.json(); setInsights(data); }
    };
    const loadHolidays = async () => {
        const res = await fetch('http://localhost:8080/api/holidays', { headers: { 'Authorization': 'Bearer ' + token } });
    ...
    const addHoliday = async () => {
        ...
        const res = await fetch('http://localhost:8080/api/holidays', { method: 'POST', headers: { 'Content-Type': 'application/json', 'Authorization': 'Bearer ' + token }, body: JSON.stringify(newHoliday) });
    ...
    const deleteHoliday = async (id) => {
        ...
        await fetch('http://localhost:8080/api/holidays/' + id, { method: 'DELETE', headers: { 'Authorization': 'Bearer ' + token } });
        loadHolidays();
    };
    const addProject = async () => {
        if (!newProject.projectName || !newProject.employeeId) { setProjectMsg('error:Project name and employee required!'); return; }
        const res = await fetch('http://localhost:8080/api/projects', { method: 'POST', headers: { 'Content-Type': 'application/json', 'Authorization': 'Bearer ' + token }, body: JSON.stringify({ ...newProject, employeeId: parseInt(newProject.employeeId) }) });
    ...
    const deleteProject = async (id) => {
        ...
        await fetch('http://localhost:8080/api/projects/' + id, { method: 'DELETE', headers: { 'Authorization': 'Bearer ' + token } });
        loadProjects();
    };
    ...
    const addEmployee = async () => {
        const res = await fetch('http://localhost:8080/api/admin/employees', { method: 'POST', headers: { 'Content-Type': 'application/json', 'Authorization': 'Bearer ' + token }, body: JSON.stringify({ ...newEmp, salary: parseFloat(newEmp.salary) }) });
        if (res.ok) {
            const saved = await res.json();
            if (newUsername && newPassword) await fetch('http://localhost:8080/api/auth/register', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ username: newUsername, password: newPassword, employeeId: String(saved.id) }) });
    ...
    const deleteEmployee = async (id) => {
        ...
        await fetch('http://localhost:8080/api/admin/employees/' + id, { method: 'DELETE', headers: { 'Authorization': 'Bearer ' + token } });
        loadEmployees();
    };
    const approveLeave = async (id) => { await fetch('http://localhost:8080/api/admin/leave/' + id + '/approve', { method: 'PUT', headers: { 'Authorization': 'Bearer ' + token } }); loadPendingLeaves(); };
    const rejectLeave = async (id) => { await fetch('http://localhost:8080/api/admin/leave/' + id + '/reject', { method: 'PUT', headers: { 'Authorization': 'Bearer ' + token } }); loadPendingLeaves(); };

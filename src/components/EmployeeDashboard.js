...
function EmployeeDashboard({ token, employeeId, onLogout }) {
    ...
    const punchIn = async () => {
        ...
        if (res.ok) {
            ...
            setAttendanceMsg('success:Punch In recorded!');
            loadAttendance();
        } else {
            setAttendanceMsg('error:Already punched in today!');
        }
    };
    ...
    const punchOut = async () => {
        ...
        if (res.ok) {
            setPunchStatus('Punched Out at ' + new Date().toLocaleTimeString());
            setAttendanceMsg('success:Punch Out recorded!');
            loadAttendance();
        } else {
            setAttendanceMsg('error:Punch in first!');
        }
    };
    ...
    const applyLeave = async () => {
        ...
        if (res.ok) {
            setLeaveMsg('success:Leave applied successfully!');
            ...
            <div style={styles.content}>
                ...
                {activeTab === 'profile' && profile && (
                    <div style={styles.card}>
                        ...
                        {[
                            ['10th School', profile.school10 || 'Not Added'],
                            ['10th Percentage', profile.percent10 || '-'],
                            ['12th School', profile.school12 || 'Not Added'],
                            ['12th Percentage', profile.percent12 || '-'],
                ...
                {activeTab === 'attendance' && (
                    <div style={styles.card}>
                        ...
                        {attendanceMsg && (
                            <p style={{ color: attendanceMsg.startsWith('success') ? '#059669' : '#dc2626', fontWeight: '500', fontSize: '14px' }}>
                                {attendanceMsg.split(':')[1]}
                            </p>
                        )}
                        ...
                        <div style={{ overflowX: 'auto' }}>
                            <table style={styles.table}>
                                ...
                                <tbody>
                                {attendance.length === 0 ? (
                                    <tr><td colSpan="4" style={{ textAlign: 'center', padding: '30px', color: '#9ca3af' }}>No records found</td></tr>
                                ) : attendance.map(a => (
                                    <tr key={a.id}>
                                        <td style={styles.td}>{a.date}</td>
                                        <td style={styles.td}>{a.punchIn || '-'}</td>
                                        <td style={styles.td}>{a.punchOut || '-'}</td>
                ...
                {activeTab === 'leave' && (
                    <div style={styles.card}>
                        ...
                        <div style={styles.formGrid}>
                            ...
                            <input className="modern-input" style={styles.input} placeholder="Reason"
                                   value={leaveForm.reason}
                                   onChange={e => setLeaveForm({ ...leaveForm, reason: e.target.value })} />
                        </div>
                        <button className="action-btn" style={styles.btnBlue} onClick={applyLeave}>Apply Leave</button>
                        {leaveMsg && (
                            <p style={{ color: '#059669', fontWeight: '500', fontSize: '14px', marginTop: '8px' }}>
                                {leaveMsg.split(':')[1]}
                            </p>
                        )}
                        {pendingCount > 0 && (
                            <p style={{ color: '#f59e0b', fontWeight: '600', fontSize: '14px' }}>Pending Leaves: {pendingCount}</p>
                        )}
                        ...
                        <div style={{ overflowX: 'auto' }}>
                            <table style={styles.table}>
                                ...
                                <tbody>
                                {leaves.length === 0 ? (
                                    <tr><td colSpan="4" style={{ textAlign: 'center', padding: '30px', color: '#9ca3af' }}>No leaves applied yet</td></tr>
                                ) : leaves.map(l => (
                                    <tr key={l.id}>
                                        <td style={styles.td}>{l.fromDate}</td>
                                        <td style={styles.td}>{l.toDate}</td>
                                        <td style={styles.td}>{l.reason}</td>
                                        <td style={{ ...styles.td, color: statusColor(l.status), fontWeight: '600' }}>
                                            {l.status}
                                        </td>
                                    </tr>
                                ))}
                ...
                {activeTab === 'changepass' && (
                    <div style={styles.card}>
                        ...
                        <input className="modern-input" style={styles.input} type="password" placeholder="Current Password"
                               value={oldPass} onChange={e => setOldPass(e.target.value)} />
                        <input className="modern-input" style={styles.input} type="password" placeholder="New Password"
                               value={newPass} onChange={e => setNewPass(e.target.value)} />
                        <input className="modern-input" style={styles.input} type="password" placeholder="Confirm New Password"
                               value={confirmPass} onChange={e => setConfirmPass(e.target.value)} />
                        <button className="action-btn" style={styles.btnBlue} onClick={changePassword}>Update Password</button>
                        {passMsg && (
                            <p style={{ color: passMsg.startsWith('success') ? '#059669' : '#dc2626', marginTop: '10px', fontWeight: '500', fontSize: '14px' }}>
                                {passMsg.split(':')[1]}
                            </p>
                        )}
                    </div>
